...
Bu sürümde **109 sunucu/birim testi**, **60 farklı ana tarayıcı senaryosu**, **3 hesap kurtarma** ve **2 genel yönetim** senaryosu doğrulandı: toplam **174 uygulama testi**. TypeScript/Vite üretim derlemesi ve üretim bağımlılık denetimi başarılı; bildirilen açık yok. Aşağıdaki v4 ve daha eski sonuçlar tarihsel kayıttır.
...
Linux CI altı kişilik görüşmede düğme etkileşimlerinin geciktiğini yakaladı; diğer 58 senaryo geçti. Son uygulama, katılımcı değişimlerinde ses analizini yeniden kurmak ve senkron ses kaynağı sorgulamak yerine asenkron WebRTC istatistiklerini paylaşır. Her katılımcının bekleyen sorgusu ayrıdır; eski yanıtlar konuşma/bağlantı göstergesini açık bırakamaz. Gerçek yerel/uzak sesin ek AudioContext olmadan ölçülmesi, desteklemeyen tarayıcılar için tek Web Audio alternatifi ve üç katılımcıda geciken sorgu/ayrılma sınırları test edildi. Değişmeden kalan altı kişilik medya/dayanıklılık senaryosu yerelde 19,6 saniyede tamamlandı. Beş ses ölçüm birim testi zaman damgası, paket ilerlemesi ve mikrofon değişimini denetler. Hesap senaryosunun mesaj seçicisi de gönderilmiş mesajı yazma kutusundan ayıracak şekilde sınırlandırıldı.

CI aynı altı kişilik senaryoyu ilk sırada, kalan 59 tarayıcı senaryosunu ikinci komutta çalıştırır; kabul kapsamı 60 senaryo olarak korunur. Bu sıra Linux medya hatalarında daha erken geri bildirim sağlar. Yerel `npm run test:e2e` tümünü birlikte çalıştırmaya devam eder.

İki mantıksal işlemciye sınırlandırılan yerel tanılamada aynı altılı test, tam iz kaydıyla 61,485; iz kaydı kapalıyken 37,163; son eylem/kaynak iz profiliyle 42,629 saniyede geçti. Bu yaklaşık ölçümde bir Chromium yardımcı süreci kendi CPU sınırını genişletti; sonuç Linux hata nedeninin kesin kanıtı değildir. Yedi tarayıcının sürekli ekran/DOM kaydının ölçüm yükünü azaltmak için yalnız altılı testte eylem ve kaynak izleri tutulur. Gerçek RTP, kamera/ekran çözümleme, kapasite, ayrılma/yeniden katılma kontrolleri ve bu kontrollerin bekleme eşikleri korundu; iki kişilik görüşme testlerinde tam görsel iz kaydı devam eder. CI hatalarında süreç CPU/bellek ölçümleri de `artifacts/media-processes.log` içine alınır. Bu kayıtlar Linux çalıştırıcısının 2 CPU ve yaklaşık 7,75 GiB belleğe sahip olduğunu doğruladı. Yedi tam uygulama bağlamı ve 30 RTC ucunu aynı makinede çalıştıran bu senaryonun CI işlem süresi 30, toplam süresi 300 saniyedir; yerelde işlem süresi 10, toplam süre 150 saniye kalır. Diğer arayüz testlerinin 10 saniyelik sınırı değişmez. Kapasite penceresi, kamera ve ekran paylaşımı tıklamaları ayrıca milisaniye olarak kaydedilir; bu paket normal kullanıcı etkileşim gecikmesi ölçümü değildir.
