...
Paket TCP/UDP3478, TLS TCP5349 ve UDP49160–49259 kullanır. Güvenlik duvarı/NAT'ta portları bire bir eşleyin. `TURN_URLS` aynı relay'i `turn:...:3478?transport=udp`, `turn:...:3478?transport=tcp`, `turns:...:5349?transport=tcp` olarak belirtir. Private/loopback/multicast hedefler engellenir; aynı sunucudaki relay oturumları için yalnız konteynerin kendi IPv4 adresi muaf tutulur. Başlangıç betiği tek yerel IPv4 adresini arayüzlerden keşfeder ve `relay-ip=PRIVATE`, `external-ip=PUBLIC/PRIVATE` üretir; belirsiz veya çok adresli topolojide başlamaz. İmaj binary file capability nedeniyle yalnızca `NET_BIND_SERVICE` eklenir. [coturn seçenekleri](https://github.com/coturn/coturn/blob/master/examples/etc/turnserver.conf).
