...
export function manageSiteAdmin(databasePath: string, operation: 'grant' | 'revoke', email: string) {
  ...
  try {
    return repo.transaction(() => {
      ...
      if (!user || !user.email_verified || !user.password_hash || user.suspended_at || repo.get('SELECT 1 FROM bot_accounts WHERE user_id=?', user.id) || (!user.admin_workspace_id && repo.get('SELECT 1 FROM workspaces WHERE id=? AND is_demo=1', user.workspace_id))) throw new Error('Mevcut, e-postası doğrulanmış, aktif ve kendi parolası olan gerçek bir hesap gerekli.');
      if (operation === 'revoke' && user.site_admin && repo.get('SELECT count(*) AS n FROM users u WHERE u.site_admin=1 AND u.suspended_at IS NULL AND u.email_verified=1 AND u.password_hash IS NOT NULL AND NOT EXISTS (SELECT 1 FROM bot_accounts b WHERE b.user_id=u.id) AND (NOT EXISTS (SELECT 1 FROM workspaces dw WHERE dw.id=u.workspace_id AND dw.is_demo=1) OR EXISTS (SELECT 1 FROM workspace_members wm JOIN workspaces w ON w.id=wm.workspace_id WHERE wm.user_id=u.id AND wm.removed_at IS NULL AND wm.suspended_at IS NULL AND w.is_demo=0))')!.n <= 1) throw new Error('Son uygulama yöneticisinin yetkisi kaldırılamaz. Önce başka bir doğrulanmış hesaba grant uygulayın.');
